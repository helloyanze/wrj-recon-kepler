...
内部基础浏览器验收已确认公共底图可显示、首次 R10 会定位到算法图层、任务航迹和工作台首屏可见。该结论只覆盖本记录列出的基础路径，不代表已经完成人工长时间动画、全部图层编辑、ZIP 导入或 Chrome/Edge 全量交互验收。
...
| `npm run build` | 通过；5278 个模块，生产构建成功 |
...
Chrome 实际复验曾暴露“公共底图未在启动时应用”和“首次算例仍停留在 San Francisco”两个问题；对应修复分别为 `f81ee55` 和 `d81e8f5`。当前命令行环境未成功产出 Chrome Headless 截图，因此最终静态截图使用 Edge Headless，不将其表述为 Chrome 与 Edge 的完整跨浏览器验收。

仍待按需人工检查：

- [ ] 算例选择器逐项打开 11 个内置算例。
- [ ] 第二、第三批依次出现、三角航向变化和降落后 3 秒淡出。
- [ ] 暂停、继续、拖动时间轴和 1×/10×/30×/60× 播放的连续交互。
- [ ] 1×/2×/4× 高度比例、倾斜、旋转、缩放及重置视角的连续交互。
- [ ] 四层显隐、透明度、宽度和颜色的完整组合检查。
- [ ] ZIP 导入、切换、刷新恢复和删除流程。
- [ ] 1920×1080 最终布局，以及 Chrome/Edge 的完整人工交互对照。
